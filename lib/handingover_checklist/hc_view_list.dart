...
class HCViewListPage extends KFDrawerContent {

  var userToken,projectData;
  HCViewListPage(userToken,projectData){
    this.userToken = userToken;
    this.projectData = projectData;
  }

  @override
  HCViewListPageState createState() => HCViewListPageState(userToken,projectData);
}

class HCViewListPageState extends State<HCViewListPage> {

  var userToken,projectData;
  HCViewListPageState(userToken,projectData){
    this.userToken = userToken;
    this.projectData = projectData;
  }
                      ...
                      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          ...
                          Expanded(
                            child: InkWell(
                              onTap: (){
                                getSelectedList(context,"Approved",4,Colors.green);
                              },
                              child: Row(
                                children: [
                                  Text("Approved",
      ...
      floatingActionButton: Padding(
        ...
        child: FloatingActionButton(
          ...
          onPressed: () async {
           final result = await Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => HCListPage(userToken,projectData)));
            print("result :: $result");
  ...
  Future<void> getSelectedList(BuildContext context, String title, int i, Color color) async {
    ...
    final result = await Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => new HCSelectedViewListPage(title,i,userToken,color,projectData)));
  }
  getChecklistCollection(BuildContext context) async{
    var url = Uri.parse(ApiClient.BASE_URL+"get_handover_checklist_collection");
    Map map ={
    ...
    if(response.statusCode == 200 || response.statusCode == 201){
      if(decoded['status']){
        setState(() {
          ...
          approved_qc_count = decoded['accepted_flats_qc_count'];
